...
Assumptions:
- Device area = 0.09 cm^2
- 1 SUN = 100 mW/cm^2
- Input power = 100 x 0.09 = 9 mW
"""
...
plot_folder = os.path.join(folder_path, "PCE_vs_Time_Plots")
...
device_area_cm2 = 0.09
sun_intensity = 1.0
incident_power_density_mW_cm2 = 100 * sun_intensity
input_power_mW = incident_power_density_mW_cm2 * device_area_cm2  # = 9 mW
...
print()
print(f"Input power used for PCE calculation: {input_power_mW:.3f} mW")
print()
...
if len(txt_files) == 0:
    ...
    processed_data = []
    global_min_pce = float("inf")
    global_max_pce = float("-inf")
    ...
    for file_path in txt_files:
        try:
            ...
            # Calculate PCE in %
            df["PCE_percent"] = (df["Power_mW"] / input_power_mW) * 100

            file_min = df["PCE_percent"].min()
            file_max = df["PCE_percent"].max()

            if file_min < global_min_pce:
                global_min_pce = file_min
            if file_max > global_max_pce:
                global_max_pce = file_max
    ...
    if len(processed_data) == 0:
        ...
        # Add small padding
        y_padding = 0.05 * (global_max_pce - global_min_pce)

        # In case values are almost identical
        if y_padding == 0:
            y_padding = 0.2

        y_min = global_min_pce - y_padding
        y_max = global_max_pce + y_padding

        print(f"Consistent y-axis range will be: {y_min:.2f} to {y_max:.2f} %")
        print()
        ...
        for filename, output_voltage, df in processed_data:
            try:
                ...
                average_pce = df["PCE_percent"].mean()
                min_pce = df["PCE_percent"].min()
                max_pce = df["PCE_percent"].max()

                summary_results.append({
                    "File": filename,
                    "Output Voltage (V)": output_voltage,
                    "Device Area (cm^2)": device_area_cm2,
                    "Sun Intensity": sun_intensity,
                    "Input Power (mW)": input_power_mW,
                    "Average Power Output (mW)": average_power,
                    "Average PCE (%)": average_pce,
                    "Minimum PCE (%)": min_pce,
                    "Maximum PCE (%)": max_pce
                })

                plt.figure(figsize=(6, 4))
                plt.plot(df["Time_s"], df["PCE_percent"], linewidth=1.5)
                ...
                output_plot_path = os.path.join(
                    plot_folder,
                    filename.replace(".txt", "_PCE_vs_Time.png").replace(".TXT", "_PCE_vs_Time.png")
                )
                ...
                print(f"Average PCE: {average_pce:.2f}%")
                print()
...
if len(summary_results) > 0:
    ...
    summary_excel_path = os.path.join(folder_path, "PCE_vs_Time_Summary.xlsx")
